...
**Note: Nuc3DMap might get poor performance when sequence depth is not enough (< 1 billion No-dup Pairs) under current version. We recommend to use Nuc3DMap with at least 1billion No-dup Pairs after Nuc3DMap nucprep (in foo.stats.summary.txt from nucprep).** If sequence depth is not enough, you could try -tl in nuctd and -ld in nucil with less statistical power :/
